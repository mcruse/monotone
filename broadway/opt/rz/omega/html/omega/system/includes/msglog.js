...
$Id: msglog.js 20101 2011-03-06 16:02:15Z bhagn $
*/
...
function get_application(){
    var e;
    msglogRef = top.document.getElementById('msglogframe').contentWindow;
    e = msglogRef.msglog.document.getElementById('application');
    return e.value
}
...
function get_type(){
    var e;
    msglogRef = top.document.getElementById('msglogframe').contentWindow;
    e = msglogRef.msglog.document.getElementById('type');
    return e.value
}
...
function handle_error(){
    ...
    if(msglogRef.control.document.getElementById("status")){
        msglogRef.control.document.getElementById("status").innerHTML = "Server is down....";
    }
    var e_status_td = msglogRef.control.document.getElementById("td_web_server_status");
    var e_status_text =  msglogRef.control.document.getElementById("web_server_status");
    server_status = 'down';
...
function update_server_status(){
    msglogRef = top.document.getElementById('msglogframe').contentWindow;
    if(msglogRef.control.document.getElementById("status")){
         msglogRef.control.document.getElementById("status").innerHTML = "Checking Server status...";
    }
    now = new Date();   
    var d = msglogRef.update_records.document;
    d.open();
    d.writeln('<html><body><script language="javascript">window.onError=null</script>')
    d.writeln('<img src="blank.png?now=' + now.getTime() + '" onError="top.document.getElementById(' + "'" + 'msglogframe' + "'" + ').contentWindow.handle_error()"></body></html>');
    d.close();   
}

function get_records_loop(){
   try{
       if (update && updating==false){
           start_update();
           update_server_status();
           setTimeout("update_location()",4000);      
       }
       setTimeout('get_records_loop()',REFRESH_RATE);
   }
   catch(e){ 
       alert(e);
       setTimeout('get_records_loop()',REFRESH_RATE);
...
function update_location(){
      ...
      var location = 'get_records.html?seq_num=' + seq_num;
      location = location + '&application=' + application;
      location = location + '&type=' + type;
      location = location + '&sort_order=' + sort_order;
      if(server_status == 'up'){
          if(msglogRef.control.document.getElementById("status")){
             msglogRef.control.document.getElementById("status").innerHTML = "Getting new log entries..";
          }
          e_status_td.setAttribute('bgcolor','#999933');
          e_status_text.innerHTML = 'WebServer Online';
          msglogRef.update_records.location = location;
       }
       else{
          server_status = 'up';
      }
