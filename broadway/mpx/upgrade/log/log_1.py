...
class LogObject_1:
    ...
    def __init__(self,name):
        ...
        try:
            ...
            self.filename = os.path.join(self.path, self.name + ".log.1")
    ...
    def _write_queue(self):
        if self._queue and self._write_lock.acquire(0):
            try:
                ...
                while self._queue:
                    values = self._queue.pop(0) + [self.data_manager.release_seq()]
...
def version_exists(name,path):
    if (os.path.exists(os.path.join(path,name) + '.log.1') or 
