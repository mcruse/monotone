##
# @fixme STOLEN FROM PYTHON 2.2!  CODE HAS UNADDRESSED SECURITY FLAWS!
...
class RModuleImporter(ihooks.ModuleImporter):
    def _safe_or_die(self, m, visited=None):
        if visited is None:
            visited = [m]
        elif m in visited:
            return
        ...
        if hasattr(m, '__file__'):
            if m.__file__[-3:] == '.so':
                if m.__name__ not in self.r_exec.ok_dynamic_modules:
                    raise ImportError(
                        "untrusted dynamic module: %s" % m.__name__
                        )
        for k in dir(m):
            a = getattr(m,k)
            if type(a) == types.ModuleType:
                self._safe_or_die(a, visited)
        return
    def import_it(self, partname, fqname, parent, force_load=0):
        global HAVE_ZIPIMPORT
        m = ihooks.ModuleImporter.import_it(
            self, partname, fqname, parent, force_load
            )
        if m:
            return m
        if HAVE_ZIPIMPORT:
            for zipfile in self.loader.default_path():
                if os.path.isfile(zipfile):
                    try:
                        zimp = zipimport.zipimporter(zipfile)
                        fqname = str(fqname)
                        if zimp.find_module(fqname):
                            m = zimp.load_module(fqname)
                            self._safe_or_die(m)
...
class RExec(ihooks._Verbose):
    ...
    ok_builtin_modules = ('audioop', 'array', 'binascii',
                          ...
                          '_hashlib', '_md5', '_sha', '_sha256', '_sha512',
                          '_random', '_struct')
    ...
    ok_posix_names = ('error', 'fstat', 'listdir', 'lstat', 'readlink',
                      ...
                      'EX_SOFTWARE', 'EX_TEMPFAIL', 'EX_UNAVAILABLE',
                      'EX_USAGE', 'F_OK', 'NGROUPS_MAX', 'O_APPEND', 'O_CREAT',
                      'O_DIRECT', 'O_DIRECTORY', 'O_DSYNC', 'O_EXCL',
                      'O_LARGEFILE', 'O_NDELAY', 'O_NOCTTY', 'O_NOFOLLOW',
                      'O_NONBLOCK', 'O_RDONLY', 'O_RDWR', 'O_RSYNC', 'O_SYNC',
                      'O_TRUNC', 'O_WRONLY', 'R_OK', 'TMP_MAX', 'WCOREDUMP',
                      'WEXITSTATUS', 'WIFEXITED', 'WIFSIGNALED', 'WIFSTOPPED',
                      'WNOHANG', 'WSTOPSIG', 'WTERMSIG', 'WUNTRACED', 'W_OK',
                      'X_OK',
                      ) 
    ...
    def r_import(self, mname, globals=None, locals=None, fromlist=None):
