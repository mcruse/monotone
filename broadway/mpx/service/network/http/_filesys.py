...
#	$Id: _filesys.py 20101 2011-03-06 16:02:15Z bhagn $
...
class os_filesystem:
	...
	def modified(self, path):
		return self.stat(path)[stat.ST_MTIME]
	
	def created(self, path):
		return self.stat(path)[stat.ST_CTIME]
	
	def get_times(self, path):
		stats = self.stat(path)
		return (stats[stat.ST_CTIME], stats[stat.ST_MTIME])
	
	def set_times(self, path, times):
		os.utime(self.translate(path), times)
	
	def match_times(self, inpath, outpath):
		"""
			Update c-time and m-time of file at 'outpath' 
			to match those of the file at 'inpath'.
		"""
		times = self.get_times(inpath)
		self.set_times(outpath, times)
