package com.cisco.ui.components
{
	 ...
	 */
	public class CiscoUITree extends Tree
	{
		...
		public function CiscoUITree()
		{
			...
			addEventListener(TreeEvent.ITEM_CLOSE,handleItemClose);
		...
		private function handleItemClose(event:TreeEvent):void
		{
		...
		*/
		...
		private function draggingOverSelf(event:DragEvent):Boolean
		{
			...
			var draggingNode:Object = itemToItemRenderer(event.dragSource.dataForFormat("treeItems")[0]);
		...
		private function internalCanDrop(event:DragEvent):Boolean
		{
			var idx:int = calculateDropIndex(event);
			var currNodeOver:Object = indexToItemRenderer(idx);
			var draggedItems:Object = event.dragSource.dataForFormat("treeItems");
			if ( draggedItems == null || draggedItems.length == 0 )
				return false;
			//var draggingNode:Object = itemToItemRenderer(draggedItems[0]);
			//if ( draggingNode == null )	return false;
			return canDropFunction(draggedItems[0], currNodeOver != null ? currNodeOver.data : null);
		}
